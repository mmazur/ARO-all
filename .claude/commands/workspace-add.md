...
Parse `$ARGUMENTS` by splitting on whitespace. The first token is the workspace name. Any remaining tokens are repo names. If there are no remaining tokens, default to `ARO-HCP ARO-Tools release`.
